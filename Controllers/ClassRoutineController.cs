...
namespace Student_Management_System.Controllers
{
    public class ClassRoutineController : Controller
    {
        ...
        public ActionResult TimeTableList(string searchBy, string searchValue)
        {
            ...
            string sqlqu = "select cr.rou_id,cr.rou_day,cr.class_id,cr.IsMaths,cr.IsMathsSlot,cr.IsEnglishLiterature,cr.IsEnglishLiteratureSlot,cr.IsEnglishGrammar,cr.IsEnglishGrammarSlot,cr.IsUrdu,cr.IsUrduSlot," + "cr.IsIslamiyat,cr.IsIslamiyatSlot,cr.IsScience,cr.IsScienceSlot,cr.IsPhysics,cr.IsPhysicsSlot,cr.IsChemistry,cr.IsChemistrySlot,cr.IsComputer,cr.IsComputerSlot,cr.IsHistory,cr.IsHistorySlot," + "cr.IsGeography,cr.IsGeographySlot,c.name,c.section from class_routine cr inner join class c on cr.class_id = c.id ";
            ...
            else
            {
                ...
                {
                    var searchByFname = jc.Where(p => p.rou_day.ToLower().Contains(searchValue.ToLower()));
                    return View(searchByFname);
                }
                else if (searchBy.ToLower() == "ClassName")
                {
                    var searchByPhone = jc.Where(p => p.ClassName.ToLower().Contains(searchValue.ToLower()));
                    return View(searchByPhone);
                }
                else if (searchBy.ToLower() == "section")
                {
                    var searchByPhone = jc.Where(p => p.ClassSection.ToLower().Contains(searchValue.ToLower()));
                    return View(searchByPhone);
                }
            }
